	                        @foreach($countries as $code => $locat)
                            	...
                            	<tr class="nk-tb-item{{ ($locat['cases'] >= 50 && !($code=='xz'||$code=='xd')) ? ' get-details' : '' }}" data-code="{{ strtolower($locat['country']['code']) }}" data-location="{{ $locat['country']['name'] }}" onclick="opencountrymodal('{{ strtolower($locat['country']['code']) }}','{{ $locat['country']['name'] }}');">
